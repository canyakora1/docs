...
To Identify user accounts on the target Windows system. This can be done using tools like `nxc` to query SMB services for user information.
...
??? example "NetExec: Shares Enumeration" Identify shared resources on the target Windows system. This can be done using tools like nxc to query SMB services for share information.
...
The ASREPRoast attack looks for users without Kerberos pre-authentication required. That means that anyone can send an AS\_REQ request to the KDC on behalf of any of those users, and receive an AS\_REP message.
